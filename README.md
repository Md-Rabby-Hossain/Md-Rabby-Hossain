### Hi there! 👋 I'm Golam Rabby  

💡 Electrical & Electronic Engineering (EEE) student @ KUET  
🔬 Passionate about IoT, Machine Learning & Embedded Systems  
📊 Exploring Data Analytics & Data Visualization 
🛠️ Languages & Tools: C | C++ | Python | Arduino
🚀 Built a drone from scratch & love hands-on projects  
✍️ Book lover & tech enthusiast  


📌 Projects:

🔹 ESP32-based Tachometer

🔹 Drone Build & Flight Controller

🔹 Data Analytics and Visualization with Python

🔹 IR Proximity Sensor using IR LED and Photodiode

🔹 Built Apps using Scratch Code to Control LED via ESP32

🔹 Build 'Text-to-Speech' App from Scratch Code

🔹 Temperature and Humidity Monitoring using DHT11 and ESP32

🔹 Digital Attendance System using R503, ESP32, OLED, and Google Sheets

🔹 Smart Ordering Device with ESP32 and OLED

🔹 Live Water Level Monitoring & Phone Notification System

🔹 AI in Embedded Systems & UAVs — Presentation and Research


📫 Let's Connect:  
[LinkedIn](www.linkedin.com/in/md-rabby-hossain-306b60324)
